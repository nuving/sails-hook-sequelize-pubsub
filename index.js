  ...
  return {
    ...
    augmentModels: function() {
      ...
      for (var identity in sails.models) {
        ...
        /** Sails Default ORM compatibility **/
        AugmentedModel.identity = identity;
        AugmentedModel.globalId = AugmentedModel.name;
        if (!AugmentedModel.primaryKey) {
          AugmentedModel.primaryKey = AugmentedModel.primaryKeyAttribute;
        }
  ...
  function getPubsubMethods () {
    ...
    return {
      ...
      room: function (id, context) {
        if (!id) {
          return sails.log.error('Must specify an `id` when calling `Model.room(id)`');
        }
      ...
      */
      ...
      introduce: function(model) {
        ...
        // Get the instance ID
        var id = _.isArray(model) ? model[this.primaryKey] || model : model;
